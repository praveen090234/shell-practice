...
if [ $USERID -ne 0 ]
then
    echo -e "$R Error: $N This user is not running with root access" $>>$LOG_FILE
    exit 1
else
    echo -e "$G This user is running with root access $N" $>>$LOG_FILE
...
if [ $? -ne 0 ]
then
    echo -e "$Y python3 is not installed.....going to install it" $>>$LOG_FILE
